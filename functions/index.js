...
//play artist
  ...
  function findBasicEvent (app) {
    ...
    getGeoData(city).then(function(r){
        var data = r;
   ...
   //     app.tell('Latitude & Longitude : ' + latitude + " , " + longitude);

        getBasicSkiddleEvents(latitude,longitude).then(function (res) {
            var data = res;
            ...
            for(let i = 0; i<pageCount;i++){
                eventList.push(results[i]['eventname']);
            }

            var eventString = eventList.join(',  ');

            app.tell("Here is a list of events happening in " + city + ": " + eventString);
  ...
  function findArtist (app){
    ...
    findSkiddleArtist(artist).then(function(res){
        ...
        for(let i = 0; i<pageCount;i++){
            eventDict[results[i]['venue']['name']] = results[i]['eventname'];
        }
